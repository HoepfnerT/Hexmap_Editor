...
class Map_Window:
    ...
    def __init__(self, GAME_FPS = 10, WINDOW_WIDTH = 1200, WINDOW_HEIGHT = 900, STARTING_SCALE = 20, TITLE = "Hexmap Editor"):
        ...
        self.BLINK                              = Tools.Blink(self.GAME_FPS/4) # Alternating between True and False
        ...
        self.GAME_WIDTH, self.GAME_HEIGHT       = (int(self.WINDOW_WIDTH-self.SIDEBAR_WIDTH), self.WINDOW_HEIGHT)
        ...
        self.SIDEBAR_RECT                       = pygame.Rect(self.GAME_WIDTH,0, self.SIDEBAR_WIDTH, self.SIDEBAR_HEIGHT)
    ...
    def change_key_to_tile(self, key, tile):
        self.MAP_HANDLER.change_key_to_tile(key, tile)
    ...
    def draw_map(self):
        # reset background
        pygame.draw.rect(self.DISPLAY, self.WHITE, self.GAME_RECT)
        # draw hexes
        self.rescale_dict(self.MAP_HANDLER.get_scale()) # check if rescaling needed
        for tile, pos, scale in self.MAP_HANDLER.get_visible_hexes(self.GAME_WIDTH, self.GAME_HEIGHT):
            if tile not in self.TILES_DICT: 
                try: self.TILES_DICT[tile] = pygame.transform.scale(pygame.image.load(f"hex_tiles/{tile}.png").convert_alpha(), self.RESCALE_IMG(scale))
                    ...
                    continue
            self.DISPLAY.blit(self.TILES_DICT[tile], pos - Vector2(scale,scale))
            #pygame.draw.polygon(self.DISPLAY, self.SHADING[tile], hex)
        #blink main select or off select
        if self.BLINK.toggle():
            ...
            if self.MAP_HANDLER.get_select(off = 1) != False: 
                for hex in self.MAP_HANDLER.get_select_coords(off=1):
                    pygame.draw.polygon(self.DISPLAY, self.BLUE, self.MAP_HANDLER.hexshape(*hex))
        #draw border
        pygame.draw.rect(self.DISPLAY, self.BLACK, self.GAME_RECT, width=self.BORDER_WIDTH)
        ...
        for key in range(10):
            ...
            if tile not in self.DISPLAY_DICT: 
                try: self.DISPLAY_DICT[tile] = pygame.transform.scale(pygame.image.load(f"hex_tiles/{tile}.png").convert_alpha(), self.RESCALE_IMG(self.SIDEBAR_TILES_DICT_SCALE))
                    ...
                    print(f"Tile {tile} not found.")
                    continue
            self.DISPLAY.blit(self.DISPLAY_DICT[tile], (self.GAME_WIDTH + self.SIDEBAR_SCALE/4 + self.SIDEBAR_SCALE/20, (5*(row)+1)*self.SIDEBAR_SCALE/4) + Vector2(col*self.SIDEBAR_SCALE,0)) 
            t_width, t_height = self.FONT.size(f"{key}")
            label = self.FONT.render(f"{key}", True, self.WHITE)
            #label.set_alpha(self.FONT_TRANSPARENCY)
            self.DISPLAY.blit(label, (self.GAME_WIDTH + self.SIDEBAR_SCALE/4, (5*(row)+1)*self.SIDEBAR_SCALE/4) + Vector2( self.SIDEBAR_SCALE/2-t_width/2 , self.SIDEBAR_SCALE/2-t_height/2 ) + Vector2(col*self.SIDEBAR_SCALE,0))
    ...
    def handle_event(self, event):
        Event_Handler.handle_event(event, self.MAP_HANDLER, self.GAME_RECT, self.SIDEBAR_RECT)